...
각 <Keyword>step</Keyword>별로 필요한 <Keyword>context</Keyword>의 타입을 제네릭으로 정의하고, <Keyword>step</Keyword> 을 키로, <Keyword>context</Keyword> 의 타입을 값으로 하는 객체 타입을 [`useFunnel()`](./use-funnel)의 제네릭으로 넘겨주세요.
