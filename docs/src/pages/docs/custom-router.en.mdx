...
`@use-funnel` stores state snapshots for each funnel step in an array, allowing for stable backward and forward navigation. Here’s how to create a custom router without using the basic functions provided by router packages.
...
Below is an example of implementing a funnel using only [`useState`](https://react.dev/reference/react/useState) without a router.
...
export const useFunnel = createUseFunnel(({ id, initialState }) => {
  ...
  return useMemo(
    () => ({
      // Return history and currentIndex to represent the current state of the funnel.
      history,
      currentIndex,
      ...
      // replace function overwrites the current state and updates the state snapshot.
      replace(state) {
        setHistory((prev) => {
...
Finally, let’s look at the `CreateUseFunnelResult<T>` interface that describes the result returned by `createUseFunnel()`. This interface includes methods for managing funnel state and transitions.
...
This interface includes various methods to manage the funnel state and handle step transitions, ensuring stable state management for your funnel.
