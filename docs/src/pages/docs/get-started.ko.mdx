...
[`useFunnel(){:tsx}`](./use-funnel.ko.mdx)에서 반환된 `step`에 따라 `context`와 `history`를 사용해요. 각 단계별로 UI를 구성하고, 필요한 상태와 이벤트를 처리할 수 있어요.
...
[`<funnel.Render />{:jsx}`](./funnel-render.ko.mdx) 컴포넌트를 사용하는 자세한 방법은 [레퍼런스](/docs/funnel-render)를 참고해주세요.
