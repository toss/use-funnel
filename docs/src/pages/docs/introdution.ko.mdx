...
```mermaid
%%{init: { 'logLevel': 'debug', 'theme': 'base', 'gitGraph': {'showBranches': true, 'showCommitLabel':true,'mainBranchName': '대출퍼널'}} }%%
      gitGraph
        commit id:"이름입력"
        commit id:"직업선택"
        branch "사업자퍼널"
        ...
        branch "본인인증퍼널"
        commit id:"토스인증"
        commit type: HIGHLIGHT id:"인증실패"
        commit id:"담보물입력"
        checkout "대출퍼널"
        merge "본인인증퍼널"
        merge "사업자퍼널" tag:"사업자입력완료"
        commit id:"담보물입력"
        commit type:REVERSE id:"가심사완료"
```
