...
[`useFunnel(){:tsx}`](./use-funnel.ko.mdx)의 반환값인 [UseFunnelResults](/docs/use-funnel#usefunnelresults)에 포함되어 있어요.
...
- `history` (`object`): 퍼널의 이동을 관리하는 객체에요. 다음  <Keyword>step</Keyword>로 이동하거나 이전  <Keyword>step</Keyword>로 돌아갈 때 사용해요. 자세한 내용은 [`FunnelHistory`](./use-funnel.ko.mdx#funnelhistory)를 참고하세요.
