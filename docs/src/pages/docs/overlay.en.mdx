...
Overlay is used to display UI such as modals, bottom sheets, etc. while maintaining the previous step as shown in the image on the right.

In the following example, when moving from the "School Input" step to the "Start Date Input" step, an overlay is used. When the `<StartDateInputBottomSheet />` component is rendered, the UI of the previous step `<SchoolInput />` is maintained.
...
**Note:** If the overlay is closed by an interaction other than the router's back button, you need to explicitly execute `close()` in the `render()` argument. When you execute `close()`, you move to the previous step through the history.
</Callout>
...
If you want to define an overlay and [transition events](/docs/transition-event) together, use the `events` property.

In the following example, the "Email Input" step uses transition events and an overlay together. Define the "Email Input Complete" and "Email Input Fail" events, and transition to the next step for each event.
...
<Callout type="warning">
When defining events, you cannot use `history` in the rendering function. Instead, you can call the defined events using `dispatch()`.
</Callout>