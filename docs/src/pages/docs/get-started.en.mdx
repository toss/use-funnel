...
Let's implement a simple sign-up flow using `@use-funnel`. You'll learn how to define the state of each step and safely manage the state step by step.
...
First, specify the object type with the key as the step and the context object as the value in the generics of [`useFunnel()`](./use-funnel). Pass the type you defined for each step in the previous step to [`useFunnel()`](./use-funnel). Specify the step and context object to be used when entering the component in the `initial` property.
...
[`useFunnel()`](./use-funnel) returns `context` and `history` based on the `step`. You can configure the UI for each step and handle the required state and events.
...
- `funnel.history.push(){:jsx}`: You can move to the next step. Pass the `step` as the first argument to `push()` and the `context` required to enter that `step` as the second argument.
- `funnel.history.replace(){:jsx}`: The basic behavior is the same as `funnel.history.push()`, but it replaces the current `step` without stacking the history.

#### Note: Implementing easily using `<funnel.Render />`

If you want to define the rendering logic for each step in one place, you can use `<funnel.Render />`. This component allows you to define the rendering logic for each step and pass the necessary data for each step.
