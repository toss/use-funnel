...
In `@use-funnel`, you need to specify the type of <Keyword>context</Keyword> required at each step (<Keyword>step</Keyword>). Let's look at various ways to define <Keyword>context</Keyword>.
...
- `guard` (`function`, optional): A function that verifies whether the data is of the context type of that step.
...
You can create a <Keyword>step</Keyword> that changes a specific key of the initial <Keyword>context</Keyword> from optional to required when moving to the next <Keyword>step</Keyword> simply.
...
Use runtime validator packages to ensure data validity and type safety. Here are examples using `zod` and `superstruct`.
